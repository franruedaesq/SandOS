...
pub const FN_GET_TOUCH_QUEUE_LEN: &str = "host_get_touch_queue_len";
pub const FN_DEQUEUE_TOUCH_EVENT: &str = "host_dequeue_touch_event";
...
/// Size in bytes of one serialized touch event payload.
pub const TOUCH_EVENT_SIZE: u32 = 16;

/// Maximum touch events returned per dequeue ABI call.
pub const MAX_TOUCH_DEQUEUE: u32 = 8;
...
/// Host touch event payload written by `host_dequeue_touch_event`.
///
/// Little-endian packed layout (16 bytes total):
/// - `[0..4]`: phase (`0=Down`, `1=Move`, `2=Up`)
/// - `[4..8]`: touch id
/// - `[8..12]`: x in display coordinates
/// - `[12..16]`: y in display coordinates
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
...
impl TouchEventData {
    #[inline]
    pub fn to_bytes(self, out: &mut [u8]) -> bool {
        if out.len() < TOUCH_EVENT_SIZE as usize {
            return false;
        }
