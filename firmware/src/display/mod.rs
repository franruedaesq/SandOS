            ...
            if !state.was_touched {
                ...
                match state.ui_mode {
                    ...
                    UiMode::TopMenu | UiMode::SubMenu(_) | UiMode::WebMenu => {
                        // Menu is on the left (x < 120). Check if touch is in the menu area.
                        if vx < 120 {
                            let slot = (vy / 40) as usize;
...
const TOP_MENU: &[MenuItem] = &[
    MenuItem { label: "Help",  action: MenuItemAction::Back },
    MenuItem { label: "Talk",  action: MenuItemAction::OpenWebMenu },
    MenuItem { label: "Info",  action: MenuItemAction::EnterSystemMonitor },
...
<<<<<<< HEAD
    if state.force_clear {
        ...
        let eye_cy_prev = 140 + state.prev_bob_y;
        let le_cx_prev = 60 + state.prev_eye_look_x;
        let re_cx_prev = 180 + state.prev_eye_look_x;
        let mouth_cy_prev = 220 + state.prev_bob_y;

        // Erase old face
        draw_eyes(oled, state.prev_expression, le_cx_prev, re_cx_prev, eye_cy_prev, state.prev_blinking, 2, Rgb565::WHITE, Rgb565::WHITE);
        draw_mouth(oled, state.prev_expression, 120, mouth_cy_prev, 2, now_ms, Rgb565::WHITE, Rgb565::WHITE);

        let blush_color = Rgb565::WHITE;
        let cheek_r = 12;
        let _ = Circle::new(Point::new(le_cx_prev - 32, eye_cy_prev + 16), (cheek_r * 2) as u32)
            .into_styled(PrimitiveStyle::with_fill(blush_color)).draw(oled);
        let _ = Circle::new(Point::new(re_cx_prev + 8, eye_cy_prev + 16), (cheek_r * 2) as u32)
            .into_styled(PrimitiveStyle::with_fill(blush_color)).draw(oled);
    }
...
=======
    ...
    state.prev_bob_y = bob_y;
>>>>>>> origin/hardware-support
    state.prev_eye_look_x = state.eye_look_x;

    // ── Eyes ──
    let eye_cy = 140 + bob_y;
    let le_cx = 60 + state.eye_look_x; // left-eye centre X
    let re_cx = 180 + state.eye_look_x; // right-eye centre X
    draw_eyes(oled, state.expression, le_cx, re_cx, eye_cy, blinking, 2, Rgb565::BLACK, Rgb565::WHITE);

    // ── Cheeks (Blush) ──
    let blush_color = Rgb565::new(31, 45, 23); // matching pink for white bg
    let _ = Circle::new(Point::new(le_cx - 32, eye_cy + 16), (cheek_r * 2) as u32)
        .into_styled(PrimitiveStyle::with_fill(blush_color)).draw(oled);
    let _ = Circle::new(Point::new(re_cx + 8, eye_cy + 16), (cheek_r * 2) as u32)
        .into_styled(PrimitiveStyle::with_fill(blush_color)).draw(oled);

    // ── Mouth ──
    let mouth_cy = 220 + bob_y;
    draw_mouth(oled, state.expression, 120, mouth_cy, 2, now_ms, Rgb565::BLACK, Rgb565::WHITE);

    // ── Status text overlay ──
    if !state.text.is_empty() {
        let text_style = MonoTextStyle::new(&FONT_6X10, Rgb565::BLACK);
<<<<<<< HEAD
        let _ = Rectangle::new(Point::new(0, 300), Size::new(DISPLAY_WIDTH, 10))
=======
        let _ = Rectangle::new(Point::new(0, 54), Size::new(DISPLAY_WIDTH, 10))
>>>>>>> origin/hardware-support
            .into_styled(PrimitiveStyle::with_fill(Rgb565::WHITE))
            .draw(oled);
        let _ = Text::new(&state.text, Point::new(0, 310), text_style).draw(oled);
    }
}
...
<<<<<<< HEAD
    let fill_on = PrimitiveStyle::with_fill(color_on);
    let fill_off = PrimitiveStyle::with_fill(color_off);
    let stroke2 = PrimitiveStyle::with_stroke(color_on, if scale > 0 { 2 } else { 1 });
=======
...
>>>>>>> origin/hardware-support
    ...
    match expr {
        ...
        EyeExpression::Heart => {
            ...
            draw_heart(oled, le_cx, eye_cy, r, color_on);
            draw_heart(oled, re_cx, eye_cy, r, color_on);
        }
        EyeExpression::Sleepy => {
...
<<<<<<< HEAD
fn draw_heart(oled: &mut TftDisplay, cx: i32, cy: i32, r: i32, color_on: Rgb565) {
    let fill_on = PrimitiveStyle::with_fill(color_on);
=======
    ...
    for dy in 0..=hr {
        ...
        if half_w > 0 {
            let _ = Line::new(
                ...
                Point::new(cx + half_w, cy + dy),
<<<<<<< HEAD
            ).into_styled(PrimitiveStyle::with_stroke(color_on, 1)).draw(oled);
=======
...
<<<<<<< HEAD
    let fill_on = PrimitiveStyle::with_fill(color_on);
    let fill_off = PrimitiveStyle::with_fill(color_off);
    let stroke = PrimitiveStyle::with_stroke(color_on, 1);
    let stroke2 = PrimitiveStyle::with_stroke(color_on, if scale > 0 { 2 } else { 1 });
=======
    let fill_on = PrimitiveStyle::with_fill(Rgb565::BLACK);
    let fill_off = PrimitiveStyle::with_fill(Rgb565::WHITE);
...
<<<<<<< HEAD
    ...
    let _ = Line::new(Point::new(120, 0), Point::new(120, 320))
...
fn render_menu_panel(oled: &mut TftDisplay, state: &FaceState) {
    ...
    for slot in 0..max_visible {
        ...
        let y_top = (slot as i32) * 40;
        if i == sel {
<<<<<<< HEAD
            let _ = Rectangle::new(Point::new(0, y_top), Size::new(119, 40))
=======
            let _ = Rectangle::new(Point::new(0, y_top), Size::new(63, 16))
>>>>>>> origin/hardware-support
                .into_styled(PrimitiveStyle::with_fill(Rgb565::BLACK))
                .draw(oled);
            let _ = Text::new(label, Point::new(8, y_top + 24), inverted_style).draw(oled);
        } else {
            let _ = Text::new(label, Point::new(8, y_top + 24), normal_style).draw(oled);
        }
    }

    // Scroll indicators
    if scroll_start > 0 {
        let _ = Text::new("\x1e", Point::new(104, 20), normal_style).draw(oled);
    }
    if scroll_start + max_visible < total {
        let _ = Text::new("\x1f", Point::new(104, 300), normal_style).draw(oled);
    }
}
...
<<<<<<< HEAD
    let eye_cy_prev = 160 + state.prev_bob_y;
    let le_cx_prev = 150 + state.prev_eye_look_x;
    let re_cx_prev = 210 + state.prev_eye_look_x;
    let mouth_cy_prev = 200 + state.prev_bob_y;
=======
    let face_changed = state.expression != state.prev_expression
        || blinking != state.prev_is_blinking
        || bob_y != state.prev_bob_y
...
>>>>>>> origin/hardware-support
    ...
    // Erase old mini face
    draw_eyes(oled, state.prev_expression, le_cx_prev, re_cx_prev, eye_cy_prev, state.prev_blinking, 1, Rgb565::WHITE, Rgb565::WHITE);
    draw_mouth(oled, state.prev_expression, 180, mouth_cy_prev, 1, now_ms, Rgb565::WHITE, Rgb565::WHITE);

    let blush_color = Rgb565::WHITE;
    let cheek_r = 6;
    let _ = Circle::new(Point::new(le_cx_prev - 16, eye_cy_prev + 8), (cheek_r * 2) as u32)
        .into_styled(PrimitiveStyle::with_fill(blush_color)).draw(oled);
    let _ = Circle::new(Point::new(re_cx_prev + 4, eye_cy_prev + 8), (cheek_r * 2) as u32)
        .into_styled(PrimitiveStyle::with_fill(blush_color)).draw(oled);
    ...
    let eye_cy = 160 + bob_y;
    let le_cx = 150 + state.eye_look_x;
    let re_cx = 210 + state.eye_look_x;

    draw_eyes(oled, state.expression, le_cx, re_cx, eye_cy, blinking, 1, Rgb565::BLACK, Rgb565::WHITE);
    ...
    // Mini mouth
    let mouth_cy = 200 + bob_y;
    draw_mouth(oled, state.expression, 180, mouth_cy, 1, now_ms, Rgb565::BLACK, Rgb565::WHITE);
}
