...
/// A raw payload to be transmitted over ESP-NOW.
pub type EspNowTxPayload = heapless::Vec<u8, { ESPNOW_MAX_PAYLOAD as usize }>;

/// Capacity of the ESP-NOW TX queue.
const ESPNOW_TX_QUEUE_DEPTH: usize = 8;

/// Shared channel: Router (Core 0) → ESP-NOW TX.
pub static ESPNOW_TX_CHANNEL: embassy_sync::channel::Channel<CriticalSectionRawMutex, EspNowTxPayload, ESPNOW_TX_QUEUE_DEPTH> =
...
pub async fn espnow_rx_task(
    ...
    loop {
        ...
        // ── Drain outgoing commands from router ──────────────────────────────
        for _ in 0..8 {
            if let Ok(payload) = ESPNOW_TX_CHANNEL.try_receive() {
                esp_now.send_async(&BROADCAST_ADDRESS, &payload).await.ok();
            } else { break; }
        }
